...
from lnrelease.session import REQUEST_STATS, Session
...
def hash_link(link: str) -> int:
    ...
    if match:
        return SALT + hash(match.group("asin"))
...
def parse(
    ...
    *,
    ...
    if not series:
        ...
        series = utils.Series("", series_title)
    ...
    date = get_attr(soup, "rpi-attribute-book_details-publication_date") or get_attr(
        soup, "rpi-attribute-audiobook_details-release-date"
    )
    if not date:
        for span in soup.find_all("span"):
            if span.string and DATE.fullmatch(span.string):
                div = span.find_parent("div", class_="rpi-attribute-content")
                if div:
                    date = div.find("div", class_="rpi-attribute-value").text.strip()
                break
    if (
        (product := soup.find(string=PRODUCT))
        and (entry := product.find_next(string=PUBLISHER))
        and (value := entry.find_parent(lambda x: x.name in ("li", "tr")).find(string=DETAILS))
    ):
