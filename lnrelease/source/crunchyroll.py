...
from lnrelease.utils import FORMATS, Info, Series
...
OMNIBUS = re.compile(
    r"(?:contains|collects)(?: novel)? volumes (?P<volume>\d+(?:\.\d)?-\d+(?:\.\d)?)"
)
...
def parse(jsn: dict) -> tuple[Series, Info] | None:
    ...
    if (
        vol := OMNIBUS.search(jsn["storedetaileddescription"]) or OMNIBUS.search(jsn["metataghtml"])
