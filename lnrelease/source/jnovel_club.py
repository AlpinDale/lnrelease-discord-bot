...
def parse(session: Session, serieskey: str, slug: str) -> set[Info]:
    ...
    params = {"format": "json", "limit": "500", "skip": "0"}
    while True:
        ...
        for index, book in enumerate(jsn["volumes"], start=1):
            ...
            info.add(Info(serieskey, link, NAME, NAME, title, index, "Digital", "", date))
    ...
    with Session() as session:
        ...
        while True:
            ...
            for serie in jsn["series"]:
                ...
                if random() > 0.5 and prev and (today - max(i.date for i in prev)).days > 365:
                    continue
                try:
