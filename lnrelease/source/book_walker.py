...
PAGES = DATA_DIR / "book_walker.csv"
...
def get_publisher(pub: str) -> str | None:
    try:
        pub = PUBLISHERS[pub]
        return pub
    except KeyError:
...
def parse(
    session: Session, link: str, links: dict[str, Info], format: str
) -> tuple[Series, Info] | None:
    ...
    series_title = soup.select_one(
        'div.product-detail-inner th:-soup-contains("Series Title") + td a'
    )
    if series_title:
        match = SERIES.fullmatch(series_title.text)
        if match:
            series_title = match.group("name").removeprefix("[AUDIOBOOK] ")
        else:
            series_title = series_title.text
    else:
        series_title = title
    if series_title.startswith("<Partial release>") or series_title.endswith(
        "(light novel serial)"
    ):
        return None
    ...
    for i, vol in enumerate(soup.select('h3:-soup-contains("Read all volumes") + div a'), start=1):
        href = vol.get("href")
        if href and link == href:
            index = i
        elif href and isinstance(href, str) and (inf := links.get(href)):
            inf.index = i
...
def scrape_cat(
    session: Session, cat: str, links: dict[str, Info], limit: int
    ...
    for i in range(1, limit + 1):
        ...
        for book in soup.select(".o-tile-list > .o-tile"):
            if any(
                "Chapter" == x.text.strip()
                for x in book.select(".m-tile-tag-box > .m-tile-tag > div")
            ):
                continue
        ...
        if pager_class and len(pager_class) > 0 and pager_class[0] == "on":
            break
