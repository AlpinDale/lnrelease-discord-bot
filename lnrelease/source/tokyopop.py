...
def parse(session: Session, link: str) -> tuple[Series, set[Info]] | None:
    ...
    if not page:
        return None
    jsn = page.json()
    if s := jsn.get("series", {}).get("series"):
        series = Series("", s[0]["name"])
        index = int(s[0]["number_in_series"])
    else:
        series = Series("", jsn["title"])
        index = 1
    read_result = read(jsn, series, index)
    if read_result:
        info.add(read_result)
    for f in jsn["formats"]:
        ...
        if not page:
            continue
        read_result = read(page.json(), series, index)
        if read_result:
            info.add(read_result)
    return series, info
