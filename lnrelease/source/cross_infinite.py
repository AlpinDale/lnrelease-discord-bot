...
from lnrelease.utils import EPOCH, Format, Info, Key, Series, Table, find_series

NAME = "Cross Infinite World"

DATA_DIR = Path(__file__).parent.parent.parent / "data"
PAGES = DATA_DIR / "cross_infinite.csv"


def get_format(s: str) -> str:
    match s:
        case "Paperback" | "Print" | "Paperback Releases" | "Print Releases":
            return "Paperback"
        case "Hardcover" | "Hardcover Releases":
            return "Hardcover"
        case "Audiobook" | "Audiobook Release":
            return "Audiobook"
        case (
            "Digital" | "Digital Releases" | "Digital Release" | "Available Now!" | "Pre-Order Now!"
        ):
            return "Digital"
        case _:
...
def parse(session: Session, link: str, skip: set[str]) -> tuple[Series, set[Info]]:
    ...
    if not page:
        raise ValueError(f"Failed to fetch {link}")
    soup = BeautifulSoup(page.content, "lxml")
    h2 = soup.select_one("div.container > div.row + div.row > div.box > h2 > strong")
    if not h2:
        raise ValueError(f"Could not find series title in {link}")
    ...
    for index, panel in enumerate(soup.find_all("div", class_="panel"), start=1):
        ...
        for format, urls in formats.items():
            ...
            info.add(Info(series.key, link, NAME, NAME, title, index, format, "", EPOCH, alts))
...
def scrape_full(series: set[Series], info: set[Info]) -> tuple[set[Series], set[Info]]:
    ...
    skip = {
        row.key
        for row in pages
        if isinstance(row, Key) and random() > 0.1 and row.date is not None and row.date < cutoff
    }
    links: dict[str, Info] = {}
    ...
    with Session() as session:
        ...
        for a in soup.select("div.row > div.box > div > a"):
            try:
                ...
                if not href or not isinstance(href, str):
                    continue
                link = urljoin("https://crossinfworld.com/", href)
