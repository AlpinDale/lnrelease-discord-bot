...
def parse(
    ...
    for inf in chain.from_iterable(links.values()):
        ...
        if titles is None or dates is None:
            continue
        title = norm(inf.title)
        if title not in titles and Series("", title) == series:
            i = Info(
                series.key,
                inf.link,
                inf.source,
        ...
        elif inf.date != EPOCH and dates and (counter := dates.get(norm(inf.title))) is not None:
            counter[inf.date] += 1
